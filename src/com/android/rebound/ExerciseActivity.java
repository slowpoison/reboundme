...
public class ExerciseActivity extends Activity implements SensorEventListener, TextToSpeech.OnInitListener {
	...
	float tempsum=0;
	float upthreshmax = (float) 0.06;
	float upthreshmin = (float) 0.03;
	float downthreshmax = (float) -0.06;
	float downthreshmin = (float) -0.03;
	...
	void ExerciseDetect ()
	{
		...
		if (movavg > upthreshmin && movavg < upthreshmax && zangle > .9 && downdetect==true && reset==false)
		{
			updetect = true;
			downdetect = false;
		...
		else if (movavg < upthreshmin && zangle > .9 && downdetect==true)
		{
			updetect = false;
			downdetect = false;
			reset=true;
		...
		else if (movavg < downthreshmin && movavg > downthreshmax && zangle < .3 && zangle > -.3 && updetect==true && reset==false)
		{
			updetect = false;
			downdetect = true;
			//reset = true;
		...
		else if (movavg < upthreshmin && movavg > downthreshmin && zangle < .3 && zangle > -.3 && reset==true )
		{
			updetect = false;
			downdetect = true;
			reset=false;
